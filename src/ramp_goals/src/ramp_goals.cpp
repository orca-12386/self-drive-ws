...
private:
    ...
    std::vector<double> curr_pos = {0.0, 0.0};
...
public:
    GoalPublisher() : Node("goal_publisher"), current_goal_index_(0)
    {
        ...
        odom_sub_ = this->create_subscription<nav_msgs::msg::Odometry>(
            "/odom", 10, std::bind(&GoalPublisher::odomCallback, this, std::placeholders::_1));
        ...
        RCLCPP_INFO(this->get_logger(), "Goal Publisher initialized with %zu goals", goals_.size());
    }
    ...
    bool check_cond()
    {
        ...
        RCLCPP_INFO(this->get_logger(), "Distance to next goal %f", distance);
        return false;
    }
    ...
    void publishNextGoal()
    {
        ...
        goal_pub_->publish(std::move(goal));

        RCLCPP_INFO(this->get_logger(), "Published goal %zu: (%.2f, %.2f)", 
...
int main(int argc, char** argv)
{
    ...
    while (rclcpp::ok() && node->hasMoreGoals())
    {
        if (node->check_cond())
            node->publishNextGoal();
        rclcpp::spin_some(node);
        rate.sleep();
    }

    RCLCPP_INFO(node->get_logger(), "All goals have been published. Exiting.");
    rclcpp::shutdown();
    return 0;
}