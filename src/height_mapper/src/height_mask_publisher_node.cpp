...
private:
    ...
    // HSV ranges for orange detection
    cv::Scalar orange_hsv_lower = cv::Scalar(10, 100, 100);  // Lower HSV for orange
    cv::Scalar orange_hsv_upper = cv::Scalar(25, 255, 255);  // Upper HSV for orange

    // RGB ranges for simulation (if needed)
    cv::Scalar orange_rgb_lower = cv::Scalar(0, 100, 200);   // Lower BGR for orange in sim
    cv::Scalar orange_rgb_upper = cv::Scalar(100, 180, 255); // Upper BGR for orange in sim

    cv::Mat hsv; // HSV converted image
    cv::Mat orange_mask; // Orange mask
    float target_v = 150.0f; // Target brightness for adjustment
    ...
    cv::Mat adjust_brightness(const cv::Mat& image, float target_v) {
        ...
        cv::Mat hsv_temp, result;
        cv::cvtColor(image, hsv_temp, cv::COLOR_BGR2HSV);
        
        std::vector<cv::Mat> hsv_channels;
        cv::split(hsv_temp, hsv_channels);
        
        // Adjust the V (brightness) channel
        hsv_channels[2] = cv::min(hsv_channels[2] * (target_v / 128.0), 255.0);
        
        cv::merge(hsv_channels, hsv_temp);
        cv::cvtColor(hsv_temp, result, cv::COLOR_HSV2BGR);
    ...
    void generate_orange_mask(const cv::Mat& image, cv::Mat& mask) {
            // For real camera, use HSV color space with brightness adjustment
            cv::Mat bright_image = adjust_brightness(image, target_v);
            cv::cvtColor(bright_image, hsv, cv::COLOR_BGR2HSV);
            cv::inRange(hsv, orange_hsv_lower, orange_hsv_upper, mask);
    ...
    void computeObstacleLabels(const cv::Mat& depth_image, sensor_msgs::msg::CameraInfo::SharedPtr camera_info, cv::Mat& obstacle_labels, std::vector<float>& max_obstacle_heights, std::vector<float>& min_obstacle_heights) {
        ...
        for (int i = 0; i < depth_image.rows; i++) {
            for (int j = 0; j < depth_image.cols; j++) {
                ...
                if (label > 0) {
                    double depth_value = static_cast<double>(depth_image.at<float>(i, j));
                    if (std::isfinite(depth_value) && depth_value > 0) {
                        Point base_point;
                        if (!convert_depth_to_point(j, i, depth_value, camera_info, base_point)){
                            continue;
                        }
                        if (this->sim){
                            base_point = cloudPointToBaselink(base_point);
    ...
    void publish_mask(sensor_msgs::msg::Image::SharedPtr rgb, sensor_msgs::msg::Image::SharedPtr depth, 
        ...
        try {
            ...
            // Generate orange mask
            generate_orange_mask(rgb_image, orange_mask);
            ...
            for (size_t i = 1; i < max_obstacle_heights.size(); i++) {
                ...
                float height_diff = abs(max_height - min_height);
                RCLCPP_INFO(this->get_logger(), "MAX HEIGHT: %f", max_height);
                RCLCPP_INFO(this->get_logger(), "MIN_HEIGHT: %f", min_height);
                RCLCPP_INFO(this->get_logger(), "HEIGHT: %f", height_diff);
                
                for (const auto& obj_type : object_types) {
                    if (height_diff > obj_type.min_height && height_diff < obj_type.max_height) {
            ...
            for (int i = 0; i < depth_image.rows; i++) {
                for (int j = 0; j < depth_image.cols; j++) {
                    ...
                    bool is_orange_pixel = orange_mask.at<uchar>(i, j) > 0;
                    ...
                    if (label > 0 && label < obstacle_types.size()) { 
                        ...
                        if (std::isfinite(depth_value) && depth_value > 0) {
                            ...
                            if (!type.empty()) {
                                ...
                                // For traffic_drum, use OR operation with orange mask
                                if (type == "traffic_drum") {
                                    if (is_orange_pixel) {
                                    ...
                                    // For other object types, use only height-based detection
                                    masks[type].at<uchar>(i, j) = 255;
                                    mask_pixel_count++;
                        ...
                        if (std::isfinite(depth_value) && depth_value > 0) {
                            ...
                            if (base_point.x > 1.5 && base_point.x < 10 && 
                                std::abs(base_point.y) < 10 && 
                                base_point.z > 0.6 && base_point.z < 1.2) {
                                
                                masks["traffic_drum"].at<uchar>(i, j) = 255;
                                mask_pixel_count++;
                            }
                        }
                    }
    ...
    void timer_callback() {
        ...
        if (recv) {
            try {
                if (!this->sim){
                    try{
                        transform_stamped = tf_buffer_->lookupTransform("odom", depth_image_msg->header.frame_id, depth_image_msg->header.stamp);
                    } catch (tf2::TransformException& ex) {
                        transform_stamped = tf_buffer_->lookupTransform("odom", depth_image_msg->header.frame_id, tf2::TimePointZero);
                    }
