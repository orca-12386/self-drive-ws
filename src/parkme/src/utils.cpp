...
struct MapPose {
  int x;
  int y;
  int cells_covered; //This is an addition to determine how much does explore lane function actually explore
  MapPose(int x_val = 0, int y_val = 0, int cells_covered = 0) : x(x_val), y(y_val),cells_covered(cells_covered){}
};
...
public:
  ...
  static MapPose exploreLane(const MapPose &start, const Map &map) {
    ...
    last_cell.cells_covered = cell_count;
    return last_cell;
  }
