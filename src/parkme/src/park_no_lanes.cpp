...
class ParkingActionServer : public rclcpp::Node {
    public:
        ...
        : Node("parking_action_server", options)
        {
            ...
            distance_ahead = this->get_parameter("distance_ahead").as_double();
            distance_angular = this->get_parameter("distance_angular").as_double();
            parking_mode = this->get_parameter("parking_mode").as_string();
            ...
            if(parking_mode == "parallel")
                RCLCPP_INFO(this->get_logger(),"Parallel Parking Node Initialized");
            else
                RCLCPP_INFO(this->get_logger(),"Pullin Parking Node Initialized");
    ...
    private:
        ...
        double distance_ahead = 0.0,log_angle = 0.0,distance_angular = 0.0,theta = 0.0;
        ...
        void pose_callback(const nav_msgs::msg::Odometry::SharedPtr msg) {
            ...
            MapPose goal_pose = get_goal_infront(msg,100);
            ...
            if(nearest_white_mp.x != -1 && explore_lane.cells_covered > 10)
            {
                ...
                double angle = Utils::getAngleRadians(nearest_white_wp,current_pose_.world_pose);
