...
private:
    ...
    void pub_parking_goal(int goal_x,int goal_y)
    {
        ...
        if (left_lane.x != -1 && right_lane.x != -1) {
            ...
            if(lane_width > 1.4*8.0)// || lane_width > 1.1*8.0)
            {
                ...
                if(left_found && !right_found)
                    pub_parking_goal(right_lane.x - 30,left_lane.y + 20);
                else if(right_found && !left_found)
                    pub_parking_goal(left_lane.x + 30,right_lane.y + 20);
                else
                {
    ...
    int search_ahead(const GridPoint& start, double yaw,bool isleft = false)
    {
        auto [fx, fy] = get_front_vector(yaw);
        auto [rx, ry] = get_front_vector(yaw + M_PI_2);
        for(int i=0;i<=10;i++){
            GridPoint candidate{
                start.x,
    ...
    std::pair<GridPoint, GridPoint> find_adjacent_lanes(const GridPoint& start, double yaw) {
        ...
        {
            right.y = left.y;
            //nothing found on the right side, look for some more
            right.x = search_ahead(right,yaw,false);
        }
        else if(right_found && !left_found)
        {
            left.y = right.y;
            left.x = search_ahead(left,yaw,true);
        }
        else if(!right_found && !left_found)
        {
