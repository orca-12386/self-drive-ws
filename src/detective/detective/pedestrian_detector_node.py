...
class PedestrianDetector(BaseDetector):
    def __init__(self):
        super().__init__('pedestrian_detector', 'detector/pedestrian', 'trained_models/Pedestrian.pt')
        
        # Override model path for pedestrians
        ...
        self.pointstamped_pub = self.create_publisher(PointStamped, "detector/pedestrian/position", 10)
        self.sim = True
            ...
            if (0 <= x < self.latest_depth_image.shape[1] and 
                ...
                if point:
                    ...
                    if self.bot_orientation is not None:
                        ...
                        self.get_logger().info(f"Detected Pedestrian: {self.detected_point.point}")
